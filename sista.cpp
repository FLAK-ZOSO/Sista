...
int main() {
    ...
    std::vector<std::shared_ptr<sista::Pawn>> pawns = {
        ...
        std::make_shared<sista::Pawn>(
            'O', sista::Coordinates(TEST_SIZE*0.2/2, TEST_SIZE*0.7),
            sista::ANSISettings(
        ...
        std::make_shared<sista::Pawn>(
            '%', sista::Coordinates(TEST_SIZE*0.5/2, TEST_SIZE*0.5),
            sista::ANSISettings(
        ...
        std::make_shared<sista::Pawn>(
            '!', sista::Coordinates(TEST_SIZE*0.7/2, TEST_SIZE*0.2),
            sista::ANSISettings(
        ...
        std::make_shared<sista::Pawn>(
            '#', sista::Coordinates(TEST_SIZE*0.8/2, TEST_SIZE*0.8),
            sista::ANSISettings(
    ...
    sista::SwappableField field(TEST_SIZE, TEST_SIZE/2);
    for (auto pawn : pawns) {
        field.addPawn(pawn);
    }
    ...
    for (int i=0; i<TEST_SIZE*TEST_SIZE/2; i++) {
